...
import { ArchiveStatusService } from '../services';

export const archiveStatusTableFetchUseCase = (params: NextTableFetchParams) => async (dispatch: Dispatch<NextTableActions>) => {
  dispatch(nextTableFetchDataStartedAction());
  const result = await ArchiveStatusService.getList(tableQueryMaker(params));
