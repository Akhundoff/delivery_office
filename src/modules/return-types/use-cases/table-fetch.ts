...
export const returnTypesTableFetchUseCase = (params: NextTableFetchParams) => async (dispatch: Dispatch<NextTableActions>) => {
  dispatch(nextTableFetchDataStartedAction());
  const result = await ReturnTypesService.getList(tableQueryMaker(params));
