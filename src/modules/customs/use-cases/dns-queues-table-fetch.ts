...
import { DnsQueuesService } from '../services';

export const dnsQueuesTableFetchUseCase = (params: NextTableFetchParams) => async (dispatch: Dispatch<NextTableActions>) => {
  dispatch(nextTableFetchDataStartedAction());
  const result = await DnsQueuesService.getList(tableQueryMaker(params));
