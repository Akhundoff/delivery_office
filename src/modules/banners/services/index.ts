...
export const BannersService = {
  ...
  activate: async (id: string | number, active: boolean): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
    const body = new FormData();
    body.append('banner_id[0]', String(id));
    body.append('active', active ? '1' : '0');
    try {
      const response = await caller('/api/admin/sliders/active', { method: 'POST', body });
      if (response.ok) return new ApiResult(200, null, null);
      return new ApiResult(400, 'Xəta baş verdi.', null);
    } catch {
      return new ApiResult(400, 'Şəbəkə xətası.', null);
