...
export const ticketTemplatesTableFetchUseCase = (params: NextTableFetchParams) => async (dispatch: Dispatch<NextTableActions>) => {
  dispatch(nextTableFetchDataStartedAction());
  const result = await TicketTemplatesService.getList(tableQueryMaker(params));
