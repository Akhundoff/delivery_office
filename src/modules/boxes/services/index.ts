...
import { IBox, IBoxFormValues, ITinyBox } from '../interfaces';
...
export const BoxesService = {
  ...
  selectBox: async (containerId: string | number): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
    const url = urlMaker('/api/admin/containers/select', { container_id: containerId });
    try {
      const response = await caller(url, { method: 'POST' });
      if (response.ok) return new ApiResult(200, null, null);
      return new ApiResult(400, 'Xəta baş verdi.', null);
  ...
  closeBox: async (trackCodes: string[], containerId?: string | number): Promise<ApiResult<200, null> | ApiResult<400 | 422, any>> => {
    const params: Record<string, any> = { track_code: trackCodes };
    if (containerId !== undefined) params.requires_declaration_container_id = containerId;
    const url = urlMaker('/api/admin/containers/close', params);
    try {
      const response = await caller(url, { method: 'POST' });
      if (response.ok) return new ApiResult(200, null, null);
      const result = await response.json();
  ...
  transferBox: async (trackCodes: string[]): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
    const url = urlMaker('/api/admin/containers/transfer', { track_code: trackCodes });
    try {
      const response = await caller(url, { method: 'POST' });
      if (response.ok) return new ApiResult(200, null, null);
      return new ApiResult(400, 'Xəta baş verdi.', null);
  ...
  getTinyBoxes: async (query: Record<string, any> = {}): Promise<ApiResult<200, ITinyBox[]> | ApiResult<400, string>> => {
    const url = urlMaker('/api/admin/containers/options', query);
    try {
      const response = await caller(url);
      if (response.ok) {
        const result = await response.json();
        return new ApiResult(
          200,
          (result || []).map((item: any) => ({ id: item.id, name: item.container_name })),
          null,
        );
      }
      return new ApiResult(400, 'Məlumatlar əldə edilə bilmədi', null);
  ...
  getMyBox: async (): Promise<ApiResult<200, IBox | null> | ApiResult<400, string>> => {
    const url = urlMaker('/api/admin/containers/me');
    try {
      const response = await caller(url);
      if (response.ok) {
        const result = await response.json();
        return new ApiResult(200, result.data ? toDomain(result.data) : null, null);
      }
      return new ApiResult(400, 'Xəta baş verdi.', null);
    } catch {
      return new ApiResult(400, 'Şəbəkə xətası.', null);
