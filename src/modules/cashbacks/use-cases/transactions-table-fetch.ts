...
import { TransactionsService } from '@modules/transactions/services';

export const makeCashbackTransactionsFetchUseCase = (cashbackId: string | number) => (params: NextTableFetchParams) => async (dispatch: Dispatch<NextTableActions>) => {
  dispatch(nextTableFetchDataStartedAction());
  const result = await TransactionsService.getList({ ...tableQueryMaker(params), cashback_id: cashbackId });
