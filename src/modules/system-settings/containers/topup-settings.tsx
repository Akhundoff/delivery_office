...
const errorKeyMap: Record<string, string> = {
  usd_balance_enabled: 'usdBalanceEnabled',
  try_balance_enabled: 'tryBalanceEnabled',
};
...
const TopupSettingsForm: FC<FormikProps<typeof defaultValues>> = ({ submitForm, isSubmitting }) => (
  <Form layout="vertical" component="div" size="large">
    <Row gutter={16}>
      <Col span={12}>
        <SwitchField name="usdBalanceEnabled" item={{ label: 'USD balans artımına icazə' }} />
      </Col>
      <Col span={12}>
        <SwitchField name="tryBalanceEnabled" item={{ label: 'TRY balans artımına icazə' }} />
      </Col>
    </Row>
...
export const TopupSettings: FC = () => {
  const { initialValues, onSubmit, isLoading } = useSettingsGroup('topup', defaultValues, fromApi, toApi, errorKeyMap, true);
