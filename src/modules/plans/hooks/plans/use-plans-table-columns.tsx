...
export const usePlansTableColumns = (): Column<IPlan>[] => {
        ...
        const items: MenuProps['items'] = [
          ...
          { type: 'divider' },
          {
            ...
            onClick: () =>
              Modal.confirm({
                title: 'Diqqət',
                content: 'Tarifi silməyə əminsinizmi?',
                okText: 'Sil',
                okType: 'danger',
                cancelText: 'Ləğv et',
                onOk: async () => {
                  const r = await PlansService.delete([original.id]);
  ...
  const baseColumns = useMemo<Column<IPlan>[]>(
    () => [
      ...
      {
        ...nextTableColumns.small,
        accessor: (r) => <Tag color={r.type === 'maye' ? 'blue' : 'default'}>{r.type === 'maye' ? 'Maye' : 'Standart'}</Tag>,
        id: 'type',
        Header: 'Tip',
      ...
      { accessor: (r) => r.tariffCategory.name || '—', id: 'tariff_category_name', Header: 'Tarif' },
