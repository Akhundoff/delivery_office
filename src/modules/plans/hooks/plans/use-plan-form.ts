...
export const usePlanForm = () => {
  ...
  const initialValues = useMemo<IPlanFormValues>(() => {
    ...
    return { countryId: "", weightFrom: "", weightTo: "", price: "", oldPrice: "", description: "", isLiquid: false, categoryId: "" };
  }, [id, detail.data]);

  const onSubmit = useCallback(async (values: IPlanFormValues, helpers: FormikHelpers<IPlanFormValues>) => {
