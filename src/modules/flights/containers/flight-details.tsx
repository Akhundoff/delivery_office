...
export const FlightDetailsContainer: FC<{ id: string }> = ({ id }) => {
  ...
  const extra = [
    ...
    ...(data.trendyol === 1 ? [<Button key='palets' type='text' icon={<Icons.AppstoreOutlined />} onClick={() => navigate(`/flights/${id}/palets`)}>Paletlər</Button>] : []),
  ];
          ...
          <DetailActions>
            <DetailActionCol>
              <Dropdown menu={{ items: statusMenuItems }}>
                <Button type='primary' ghost icon={<Icons.EditFilled />}>Statusu dəyiş</Button>
              </Dropdown>
              {data.trendyol === 1 && (
                <Dropdown menu={{ items: trendyolStatusMenuItems }}>
                  <Button type='primary' ghost icon={<Icons.EditFilled />}>Trendyol Statusu dəyiş</Button>
                </Dropdown>
              )}
