...
export const WarehouseService = {
  getHandoverQueues: async (query: Record<string, any> = {}): Promise<ApiResult<200, { data: IHandoverQueue[]; total: number; activeTasks: number[] }> | ApiResult<400, string>> => {
    const { state_id, ...rest } = query;
    const url = urlMaker('/api/admin/handover/list', { ...rest, state_id: typeof state_id === 'string' ? [state_id] : state_id });
    try {
      const response = await caller(url);
      if (response.ok) {
        const result = await response.json();
        const data = (result.data || []).map(toDomain);
        const activeTasks: number[] = (result.param || []).map((t: any) => t.id);
        return new ApiResult(200, { data, total: result.total || data.length, activeTasks }, null);
      }
