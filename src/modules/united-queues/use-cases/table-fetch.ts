...
import { UnitedQueuesService } from '../services';

export const unitedQueuesTableFetchUseCase = (params: NextTableFetchParams) => async (dispatch: Dispatch<NextTableActions>) => {
  dispatch(nextTableFetchDataStartedAction());
  const result = await UnitedQueuesService.getList(tableQueryMaker(params));
