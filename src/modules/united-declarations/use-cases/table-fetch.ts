...
export const unitedDeclarationsTableFetchUseCase = (params: NextTableFetchParams) => async (dispatch: Dispatch<NextTableActions>) => {
  dispatch(nextTableFetchDataStartedAction());
  const result = await UnitedDeclarationsService.getList(tableQueryMaker(params));
