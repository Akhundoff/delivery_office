...
export const emailNotificationsTableFetchUseCase = (params: NextTableFetchParams) => async (dispatch: Dispatch<NextTableActions>) => {
  dispatch(nextTableFetchDataStartedAction());
  const result = await EmailNotificationsService.getList(tableQueryMaker(params));
