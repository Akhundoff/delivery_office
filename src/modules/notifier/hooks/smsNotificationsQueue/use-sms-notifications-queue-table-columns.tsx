...
const SmsQueueStatusCell: FC<{ original: ISmsNotification }> = ({ original }) => {
    const [visible, setVisible] = useState(false);
    const { data } = useQuery(
        ['notifier', 'sms', original.id, 'status'],
        () => SmsNotificationsQueueService.getStatus(original.id).then((r) => (r.status === 200 ? r.data : '')),
        { enabled: visible },
    );
    return (
...
export const useSmsNotificationsQueueTableColumns = (): Column<ISmsNotification>[] => {
    const handleToggleActive = useCallback(async (id: number, isActive: boolean) => {
        const result = await SmsNotificationsQueueService.toggleIsActive(id, isActive);
    ...
    return useMemo(
        () => [
            ...
            },
            {
                ...
                id: 'sent',
                accessor: (r) => r.sent,
                Cell: ({ row: { original } }: any) => <SmsQueueStatusCell original={original} />,
            },
