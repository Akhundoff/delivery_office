...
import { useModels } from '@modules/models/hooks/models/use-models';
  ...
  return useMemo<Column<INotificationTemplate>[]>(
    () => [
      ...
      },
      {
        ...
        Header: 'Model',
        Filter: ({ column: { setFilter, filterValue } }: any) => (
          <Select allowClear style={{ width: '100%' }} showSearch onChange={setFilter} value={filterValue} loading={models.isLoading}>
            {models.data?.map((m) => (
      ...
      },
      {
        ...
        Header: 'Status',
        Filter: ({ column: { setFilter, filterValue } }: any) => (
          <Select allowClear style={{ width: '100%' }} showSearch onChange={setFilter} value={filterValue} loading={statuses.isLoading}>
            {statuses.data?.map((s) => (
    ...
    [navigate, handleToggleActive, handleRemove, models.data, models.isLoading, statuses.data, statuses.isLoading],
  );
};
