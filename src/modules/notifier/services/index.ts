...
export const SmsNotificationsService = {
    getList: async (query: Record<string, any> = {}): Promise<ApiResult<200, ListResponse<ISmsNotification>> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/sms', { page: 1, per_page: 20, ...query });
        try {
            const res = await caller(url);
            if (res.ok) {
                const result = await res.json();
                return new ApiResult(200, { data: (result.data as ISmsNotificationPersistence[] || []).map(SmsNotificationMapper.toDomain), total: result.total ?? 0 }, null);
            }
...
export const WhatsappNotificationsService = {
    getList: async (query: Record<string, any> = {}): Promise<ApiResult<200, ListResponse<ISmsNotification>> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/whatsapp', { page: 1, per_page: 20, ...query });
        try {
            const res = await caller(url);
            if (res.ok) {
                const result = await res.json();
...
export const EmailNotificationsService = {
    getList: async (query: Record<string, any> = {}): Promise<ApiResult<200, ListResponse<IEmailNotification>> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/mail', { page: 1, per_page: 20, ...query });
        try {
            const res = await caller(url);
            if (res.ok) {
                const result = await res.json();
                return new ApiResult(200, { data: (result.data as IEmailNotificationPersistence[] || []).map(EmailNotificationMapper.toDomain), total: result.total ?? 0 }, null);
            }
            return new ApiResult(400, 'Xəta baş verdi.', null);
        } catch {
            return new ApiResult(400, 'Şəbəkə xətası.', null);
...
export const NotificationTemplatesService = {
    getList: async (query: Record<string, any> = {}): Promise<ApiResult<200, ListResponse<INotificationTemplate>> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/templates/list', { page: 1, per_page: 20, ...query });
        try {
            const res = await caller(url);
            if (res.ok) {
                const result = await res.json();
                return new ApiResult(200, { data: (result.data as INotificationTemplatePersistence[] || []).map(NotificationTemplateMapper.toDomain), total: result.total ?? 0 }, null);
            }
            return new ApiResult(400, 'Xəta baş verdi.', null);
        } catch {
            return new ApiResult(400, 'Şəbəkə xətası.', null);
    ...
    toggleActive: async (ids: number[], isActive: boolean): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/templates/active', { template_id: ids, active: isActive ? 1 : 0 });
        try {
...
const buildBulkSmsBody = (dto: ISendBulkSmsNotificationDto): Record<string, any> => {
    const typeMap: Record<string, string> = {
        allUsers: 'users', userBirthday: 'user_birthday', userIds: 'user_names', userMonthlyLimit: 'user_limit',
        userPhoneNumbers: 'user_numbers', orderStatus: 'orders_state', declarationStatus: 'declarations_state',
        courierStatus: 'couriers_state', customsStatus: 'customs_state', flightId: 'flight_id',
    };
    ...
    return {
        ...
        'user_id[]': dto.user.ids, flight_id: dto.flightId, amount: dto.user.monthlyLimit,
        'number[]': dto.user.phoneNumbers, 'state_id[]': statusIds, customs: dto.customsDeclarationStatusId,
        d: dto.customsStatusId, 'country_id[]': dto.countryIds, 'branch_id[]': dto.branchIds,
    };
};
...
const buildBulkEmailBody = (dto: ISendBulkEmailNotificationDto): Record<string, any> => {
    const typeMap: Record<string, string> = {
        allUsers: 'users', userBirthday: 'user_birthday', userIds: 'user_names', userMonthlyLimit: 'user_limit',
        userEmails: 'user_emails', orderStatus: 'orders_state', declarationStatus: 'declarations_state',
        courierStatus: 'couriers_state', customsStatus: 'customs_state', flightId: 'flight_id',
    };
    const statusIds = dto.type === 'orderStatus' ? dto.orderStatusIds : dto.type === 'declarationStatus' ? dto.declarationStatusIds : dto.type === 'courierStatus' ? dto.courierStatusIds : [];
    return {
        search_id: typeMap[dto.type] ?? '', template_id: dto.templateId,
        planned_at: dto.plannedAt ? dto.plannedAt.format?.('YYYY-MM-DD HH:mm:ss') ?? dto.plannedAt : '',
        'user_id[]': dto.user.ids, flight_id: dto.flightId, amount: dto.user.monthlyLimit,
        'email[]': dto.user.emails, 'state_id[]': statusIds, customs: dto.customsDeclarationStatusId,
        d: dto.customsStatusId, 'country_id[]': dto.countryIds, 'branch_id[]': dto.branchIds,
    };
};
...
const buildBulkWhatsappBody = (dto: ISendBulkWhatsappNotificationDto): Record<string, any> => {
    ...
    const statusIds = dto.type === 'orderStatus' ? dto.orderStatusIds : dto.type === 'declarationStatus' ? dto.declarationStatusIds : dto.type === 'courierStatus' ? dto.courierStatusIds : [];
    return {
        search_id: typeMap[dto.type] ?? '', template_id: dto.templateId, body: dto.body,
        planned_at: dto.plannedAt ? dto.plannedAt.format?.('YYYY-MM-DD HH:mm:ss') ?? dto.plannedAt : '',
        'user_id[]': dto.user.ids, flight_id: dto.flightId, amount: dto.user.monthlyLimit,
        'number[]': dto.user.phoneNumbers, 'state_id[]': statusIds, customs: dto.customsDeclarationStatusId,
        d: dto.customsStatusId, 'country_id[]': dto.countryIds, 'branch_id[]': dto.branchIds,
...
export const SmsNotificationsQueueService = {
    getList: async (query: Record<string, any> = {}): Promise<ApiResult<200, { data: ISmsNotification[]; total: number }> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/sms_queues', { page: 1, per_page: 20, ...query });
        try {
            const res = await caller(url);
            if (res.ok) {
                const result = await res.json();
            ...
            return new ApiResult(400, 'Xəta baş verdi.', null);
        } catch { return new ApiResult(400, 'Şəbəkə xətası.', null); }
    },
    toggleIsActive: async (id: number | string, isActive: boolean): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/sms_active', { sms_id: id, active: isActive ? 1 : 0 });
        try {
            const res = await caller(url);
            if (res.ok) return new ApiResult(200, null, null);
            return new ApiResult(400, 'Xəta baş verdi.', null);
...
export const EmailNotificationsQueueService = {
    getList: async (query: Record<string, any> = {}): Promise<ApiResult<200, { data: IEmailNotification[]; total: number }> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/mail_queues', { page: 1, per_page: 20, ...query });
        try {
            const res = await caller(url);
            if (res.ok) {
                const result = await res.json();
    ...
    toggleIsActive: async (id: number | string, isActive: boolean): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/mail_active', { mail_id: id, active: isActive ? 1 : 0 });
        try {
            const res = await caller(url);
            if (res.ok) return new ApiResult(200, null, null);
            return new ApiResult(400, 'Xəta baş verdi.', null);
...
export const WhatsappNotificationsQueueService = {
    getList: async (query: Record<string, any> = {}): Promise<ApiResult<200, { data: ISmsNotification[]; total: number }> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/whatsapp_queues', { page: 1, per_page: 20, ...query });
        try {
            const res = await caller(url);
            if (res.ok) {
                const result = await res.json();
    ...
    toggleIsActive: async (id: number | string, isActive: boolean): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/whatsapp_active', { whatsapp_id: id, active: isActive ? 1 : 0 });
        try {
            const res = await caller(url);
            if (res.ok) return new ApiResult(200, null, null);
            return new ApiResult(400, 'Xəta baş verdi.', null);
    ...
    getStatus: async (id: number | string): Promise<ApiResult<200, string> | ApiResult<400, string>> => {
        const url = urlMaker('/api/admin/archive/whatsapp_status', { whatsapp_id: id });
        try {
            const res = await caller(url);
            if (res.ok) return new ApiResult(200, await res.text(), null);
            return new ApiResult(400, 'Xəta baş verdi.', null);
...
export const BulkSmsNotificationService = {
    ...
    send: async (dto: ISendBulkSmsNotificationDto): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
        ...
        try {
            const res = await caller(url, { method: 'POST', body: buildFormData(buildBulkSmsBody(dto)) });
            if (res.ok) return new ApiResult(200, null, null);
            return new ApiResult(400, 'Xəta baş verdi.', null);
        } catch { return new ApiResult(400, 'Şəbəkə xətası.', null); }
    },
...
export const BulkEmailNotificationService = {
    ...
    send: async (dto: ISendBulkEmailNotificationDto): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
        ...
        try {
            const res = await caller(url, { method: 'POST', body: buildFormData(buildBulkEmailBody(dto)) });
            if (res.ok) return new ApiResult(200, null, null);
            return new ApiResult(400, 'Xəta baş verdi.', null);
        } catch { return new ApiResult(400, 'Şəbəkə xətası.', null); }
    },
...
export const BulkWhatsappNotificationService = {
    ...
    send: async (dto: ISendBulkWhatsappNotificationDto): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
        ...
        try {
            const res = await caller(url, { method: 'POST', body: buildFormData(buildBulkWhatsappBody(dto)) });
            if (res.ok) return new ApiResult(200, null, null);
            return new ApiResult(400, 'Xəta baş verdi.', null);
        } catch { return new ApiResult(400, 'Şəbəkə xətası.', null); }
    },
...
export const BulkMobileNotificationService = {
    ...
    send: async (dto: ISendBulkMobileNotificationDto): Promise<ApiResult<200, null> | ApiResult<400, string>> => {
        ...
        try {
            const res = await caller(url, { method: 'POST', body: buildMobileSendBody(dto) });
            if (res.ok) return new ApiResult(200, null, null);
            return new ApiResult(400, 'Xəta baş verdi.', null);
        } catch {
            return new ApiResult(400, 'Şəbəkə xətası.', null);
