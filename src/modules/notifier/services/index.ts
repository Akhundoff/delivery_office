...
const buildMobileSendBody = (dto: ISendBulkMobileNotificationDto): FormData => {
  ...
  if (dto.plannedAt)
    fd.append(
      "planned_at",
      typeof dto.plannedAt === "string"
        ? dto.plannedAt
        : dto.plannedAt.format("YYYY-MM-DD HH:mm:ss"),
    );
  ...
  if (dto.customsDeclarationStatusId)
    fd.append("customs", dto.customsDeclarationStatusId);
...
export const SmsNotificationsService = {
  getList: async (
    ...
    ApiResult<200, ListResponse<ISmsNotification>> | ApiResult<400, string>
  > => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return new ApiResult(
          200,
          {
            data: ((result.data as ISmsNotificationPersistence[]) || []).map(
              SmsNotificationMapper.toDomain,
            ),
...
export const EmailNotificationsService = {
  getList: async (
    ...
    ApiResult<200, ListResponse<IEmailNotification>> | ApiResult<400, string>
  > => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return new ApiResult(
          200,
          {
            data: ((result.data as IEmailNotificationPersistence[]) || []).map(
              EmailNotificationMapper.toDomain,
            ),
            total: result.total ?? 0,
          },
          null,
...
export const NotificationTemplatesService = {
  getList: async (
    ...
    ApiResult<200, ListResponse<INotificationTemplate>> | ApiResult<400, string>
  > => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return new ApiResult(
          ...
          {
            data: (
              (result.data as INotificationTemplatePersistence[]) || []
  ...
  getById: async (
    ...
    ApiResult<200, INotificationTemplate> | ApiResult<400, string>
  > => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return new ApiResult(
          200,
          NotificationTemplateMapper.toDomain(
            result.data as INotificationTemplatePersistence,
          ),
          null,
        );
  ...
  create: async (
    dto: ICreateNotificationTemplateDto,
  ): Promise<ApiResult<200, number> | ApiResult<400, string>> => {
    const url = urlMaker(
      dto.id ? "/api/admin/templates/edit" : "/api/admin/templates/create",
    );
    try {
      const fd = new FormData();
...
const buildBulkSmsBody = (
  dto: ISendBulkSmsNotificationDto,
): Record<string, any> => {
  const typeMap: Record<string, string> = {
    allUsers: "users",
    userBirthday: "user_birthday",
  ...
  const statusIds =
    dto.type === "orderStatus"
      ? dto.orderStatusIds
      : dto.type === "declarationStatus"
        ? dto.declarationStatusIds
        : dto.type === "courierStatus"
          ? dto.courierStatusIds
          : [];
  return {
    search_id: typeMap[dto.type] ?? "",
    body: dto.body,
    planned_at: dto.plannedAt
      ? (dto.plannedAt.format?.("YYYY-MM-DD HH:mm:ss") ?? dto.plannedAt)
      : "",
...
const buildBulkEmailBody = (
  dto: ISendBulkEmailNotificationDto,
): Record<string, any> => {
  const typeMap: Record<string, string> = {
    allUsers: "users",
    userBirthday: "user_birthday",
...
const buildBulkWhatsappBody = (
  dto: ISendBulkWhatsappNotificationDto,
): Record<string, any> => {
  const typeMap: Record<string, string> = {
    allUsers: "users",
    userBirthday: "user_birthday",
...
const buildFormData = (params: Record<string, any>): FormData => {
  ...
  Object.entries(params).forEach(([key, val]) => {
    if (Array.isArray(val))
      val.forEach(
        (v) => v !== undefined && v !== "" && fd.append(key, String(v)),
      );
    else if (val !== undefined && val !== "")
      fd.append(key.replace("[]", ""), String(val));
  });
  return fd;
};
...
export const SmsNotificationsQueueService = {
  getList: async (
    ...
    | ApiResult<200, { data: ISmsNotification[]; total: number }>
  ...
  getStatus: async (
    id: number | string,
  ): Promise<ApiResult<200, string> | ApiResult<400, string>> => {
...
export const EmailNotificationsQueueService = {
  getList: async (
    ...
    | ApiResult<200, { data: IEmailNotification[]; total: number }>
    | ApiResult<400, string>
  > => {
...
export const SmsBalanceService = {
  getBalance: async (): Promise<
    ApiResult<200, number> | ApiResult<400, string>
  > => {
...
export const BulkSmsNotificationService = {
  getUsers: async (
    ...
    ApiResult<200, { data: any[]; total: number }> | ApiResult<400, string>
  > => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const total =
          typeof result.data === "number" ? result.data : data.length;
...
export const BulkEmailNotificationService = {
  ...
  > => {
    const url = urlMaker(
      "/api/admin/bulk_mail/getlist",
      buildBulkEmailBody(dto),
    );
    try {
      const res = await caller(url);
      if (res.ok) {
...
export const BulkWhatsappNotificationService = {
  ...
  > => {
    const url = urlMaker(
      "/api/admin/bulk_whatsapp/getlist",
      buildBulkWhatsappBody(dto),
    );
    try {
      const res = await caller(url);
      if (res.ok) {
...
export const BulkMobileNotificationService = {
  getUsers: async (
    dto: ISendBulkMobileNotificationDto,
  ): Promise<
    ApiResult<200, { data: any[]; total: number }> | ApiResult<400, string>
  > => {
    ...
    try {
      ...
      if (res.ok) {
        const result = await res.json();
        const data =
          typeof result.data.data === "number" ? [] : (result.data.data ?? []);
        const total =
          typeof result.data.data === "number" ? result.data.data : data.length;
        return new ApiResult(200, { data, total }, null);
      }
      return new ApiResult(400, "Xəta baş verdi.", null);
    } catch {
...
export const StatusesByModelService = {
  getListByModelId: async (
    modelId: number | string,
  ): Promise<
    ApiResult<200, { id: number; name: string }[]> | ApiResult<400, string>
  > => {
