...
import { AzerpostQueuesService } from '../services';

export const azerpostQueuesTableFetchUseCase = (params: NextTableFetchParams) => async (dispatch: Dispatch<NextTableActions>) => {
  dispatch(nextTableFetchDataStartedAction());
  const result = await AzerpostQueuesService.getList(tableQueryMaker(params));
