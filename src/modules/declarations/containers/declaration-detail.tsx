...
export const DeclarationDetail: FC<{ id: string }> = ({ id }) => {
  ...
  const boxTransfersDefaultState = useMemo(() => ({ filters: [{ id: 'declaration_id', value: data?.id }], hiddenColumns: ['track_code'] }), [data?.id]);
  ...
  const tags = (
    <TagSpace size={8}>
      ...
      {data.isCommercial ? <Tag color="blue">Kommersial bağlama</Tag> : <Tag>Vətəndaş bağlaması</Tag>}
    ...
    <DetailPage>
      <DetailRow>
        <DetailCol xs={24}>
          ...
          {!!data.editable && <Alert message="Bu bağlama məlumatları natamam doldurulduğundan gömrüyə göndərilməyib." type="warning" showIcon style={{ marginTop: 12 }} />}
        </DetailCol>
        <DetailCol xs={24}>
          <DetailActions>
            <DetailActionCol>
              ...
              <Dropdown
                overlay={
                  <Menu>
                    <Menu.Item disabled={waybillIsDisabled} onClick={printWaybill}>
                      Yol vərəqi
                    </Menu.Item>
                    <Menu.Item onClick={printProformaInvoice}>Proforma invoice</Menu.Item>
                    <Menu.Item disabled={!data.handoverTaskId} onClick={() => printHandoverCheck(data)}>
                      Təhvil sənədi
                    </Menu.Item>
            ...
            <DetailActionCol>{ordersNode}</DetailActionCol>
          </DetailActions>
        </DetailCol>
            ...
            <DetailCol xs={24}>
              <DetailCard title="Ümumi məlumat">
                <DetailDescriptions>
                  ...
                  {!!data.partnerName && <DetailDescriptions.Item label="Partnyor">{data.partnerName}</DetailDescriptions.Item>}
                  ...
                  {data.voen && <DetailDescriptions.Item label="VÖEN">{data.voen}</DetailDescriptions.Item>}
                  ...
                  {data.causerId && <DetailDescriptions.Item label="Düzəliş edən">{data.causerId}</DetailDescriptions.Item>}
                </DetailDescriptions>
              </DetailCard>
            </DetailCol>
            <DetailCol xs={24}>
          ...
          <DetailRow>
            <DetailCol xs={24}>
              <DetailCard title="Qiymətlər">
                <DetailDescriptions>
                  ...
                  <DetailDescriptions.Item label="Çatdırılma qiyməti">{data.deliveryPrice ? `${data.deliveryPrice.toFixed(2)} ${CurrencySymbols.USD}` : 'Qeyd olunmayıb'}</DetailDescriptions.Item>
                  <DetailDescriptions.Item label="Ödənilmə statusu">{data.paid ? 'Ödənilib' : 'Ödənilməyib'}</DetailDescriptions.Item>
            ...
            <DetailCol xs={24}>
              <DetailCard title="Ölçülər">
                <DetailDescriptions>
                  <DetailDescriptions.Item label="Çəki">{data.weight ? `${data.weight.toFixed(2)} kq` : 'Qeyd olunmayıb'}</DetailDescriptions.Item>
                  <DetailDescriptions.Item label="Volumn">{data.trendyolLogs?.volume || 'Qeyd olunmayıb'}</DetailDescriptions.Item>
                  <DetailDescriptions.Item label="En">{data.depth ? `${data.depth.toFixed(2)} sm` : 'Qeyd olunmayıb'}</DetailDescriptions.Item>
                  <DetailDescriptions.Item label="Uzunluq">{data.width ? `${data.width.toFixed(2)} sm` : 'Qeyd olunmayıb'}</DetailDescriptions.Item>
                  <DetailDescriptions.Item label="Hündürlük">{data.height ? `${data.height.toFixed(2)} sm` : 'Qeyd olunmayıb'}</DetailDescriptions.Item>
                </DetailDescriptions>
              </DetailCard>
            </DetailCol>
            <DetailCol xs={24}>
              <DetailCard title="Yerləşmə">
                <DetailDescriptions>
                  {data.branch && <DetailDescriptions.Item label="Filial">{data.branch.name || 'Qeyd olunmayıb'}</DetailDescriptions.Item>}
                  ...
                  <DetailDescriptions.Item label="Filial göndərişi">
                    <Row>
                      <Col span={24}>
                        <Typography.Text>{data.parcelSorting?.id ? `${data.parcelSorting.id} (${data.parcelSorting.state_name ?? ''})` : 'Çeşidlənməyib'}</Typography.Text>
                      </Col>
                      <Col span={24}>
      ...
      <Modal width={992} title="Yeşik transferləri" open={isContainerTransfersModalVisible} onCancel={closeContainerTransfersModal} footer={null}>
        {boxTransfersFetch && (
          <NextTableProvider
            key={`declaration-${data.id}-transfers`}
            context={BoxTransfersTableContext}
            onFetch={boxTransfersFetch}
            name="declaration-box-transfers-table"
            useCache={false}
            defaultState={boxTransfersDefaultState}
          >
            <BoxTransfersTable />
          </NextTableProvider>
        )}
          ...
          >
            <Form layout="vertical" component="div" size="large">
              <Form.Item label="Axtarış səbəbi">
                <Input.TextArea placeholder="Axtarış səbəbini daxil edin..." rows={4} maxLength={250} showCount value={values.descr} onChange={(e) => setFieldValue('descr', e.target.value)} />
              </Form.Item>
